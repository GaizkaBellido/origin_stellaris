 ...
 * Programa principal
 */
int main_demo_6(void)
{
	int animacion_lineas = 1;
	int retardo = 10000;
	...
	for(;;)
	{
		...
		if( libMU_Button_Pressed(BUTTON_SELECT) ) {
			if( animacion_lineas ) {
				Baile_de_elipses_inicializa();
				animacion_lineas = 0; retardo = 40000;
			}else{
				Baile_de_lineas_inicializa();
				animacion_lineas = 1; retardo = 1;
			}
		}
	}	
}
