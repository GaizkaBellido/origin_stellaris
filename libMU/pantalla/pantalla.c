...
libMU_Display_MemoryImage_t		libMU_Display_LocalImage = {
		libMU_Display_ImageBuffer, DISPLAY_RES_X/2, DISPLAY_RES_Y, sizeof(libMU_Display_ImageBuffer) };
						...
						int lX1, int lY1, int lX2, int lY2, unsigned int intensity )
{
    ...
    if( two_b2 == 0 || two_a2 == 0 ) {
    	libMU_Display_MemoryImage_DrawLine( mi, lX1, lX2, lY1, lY2, intensity );
    	return;
    }
 ...
 * @param intensity	intensidad del color (de 0 a 15)
 */
static inline void libMU_Display_MemoryImage_DrawPixelLocal( int lX, int lY, int intensity)
{
    if(lX & 1)
    {
        libMU_Display_ImageBuffer[(lY * libMU_Display_LocalImage.width) + (lX / 2)] =
            ((libMU_Display_ImageBuffer[(lY * libMU_Display_LocalImage.width) + (lX / 2)] & 0xf0) |
             (intensity & 0xf));
    }
    else
    {
        libMU_Display_ImageBuffer[(lY * libMU_Display_LocalImage.width) + (lX / 2)] =
            ((libMU_Display_ImageBuffer[(lY * libMU_Display_LocalImage.width) + (lX / 2)] & 0x0f) |
             ((intensity & 0xf) << 4));
    }
}
 ...
 */
void
libMU_Display_MemoryImage_DrawLineLocal(int lX1, int lY1, int lX2, int lY2, unsigned int intensity)
{
    long lError, lDeltaX, lDeltaY, lYStep;
    tBoolean bSteep;
    ...
    for(; lX1 <= lX2; lX1++)
    {
        ...
        {
            // Plot this point of the line, swapping the X and Y coordinates.
            libMU_Display_MemoryImage_DrawPixelLocal( lY1, lX1, intensity );
        }
        else
        {
            // Plot this point of the line, using the coordinates as is.
            libMU_Display_MemoryImage_DrawPixelLocal( lX1, lY1, intensity );
        }
        // Increment the error term by the Y delta.
        lError += lDeltaY;
